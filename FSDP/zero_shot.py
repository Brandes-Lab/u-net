...
Updated for DDP compatibility with bf16 autocast support.
"""
...
def run_batched_vep_inference(
    ...
    Args:
        ...
        tokenizer: Tokenizer for the model
        device: Device to run inference on
        valid_items: List of prepared VEP items
    ...
    for batch_start in range(0, len(sorted_indices), batch_size):
        ...
        for b, orig_idx in enumerate(batch_indices):
            ...
            # Cast to float32 for softmax stability
            probs = F.softmax(logits[b, mask_pos].float(), dim=-1)
            log_odds = (torch.log(probs[alt_id] + eps) - torch.log(probs[ref_id] + eps)).item()
...
def run_vep_eval_batched(
    ...
    Args:
        ...
        tokenizer: Tokenizer for the model
        device: Device to run inference on
        df: DataFrame with VEP data
        step: Current training step (for logging)
        csv_path: Optional path to save results CSV
        min_length: Minimum sequence length
        max_length: Maximum sequence length
        batch_size: Batch size for inference
    ...
    scores = run_batched_vep_inference(
        model, tokenizer, device, valid_items, batch_size, max_length, use_amp=use_amp
    )
...
def run_vep_eval(
    ...
    By default uses batched evaluation for speed with AMP enabled.
    ...
    Args:
        ...
        max_length: Maximum sequence length
        batch_size: Batch size for inference
        use_batched: Whether to use batched inference
        use_amp: Whether to use automatic mixed precision
    ...
    if use_batched:
        return run_vep_eval_batched(
            ...
            min_length, max_length, batch_size, use_amp=use_amp
        )
    else:
