...
class TransformerConfig:
    ...
    heads: int = 8
    pair_dim: int = 32
    rope_max_position: int = 2048   # large  upper bound
    target_pair_len: int = 32       # preferred pair grid; adapts at runtime
    repeat_factor: int = 1          # no longer used to “force” S; keep for compat
    pos_feat_dim: int = 64          # positional feature dimension (must be even)
...
class TransformerTower(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.cfg.depth):
            ...
            attn_bias = self.attn_bias_block(pair_x, P, S)  # [B,H,S,S]   
...
class sequence_to_pair_block(nn.Module):
    def __init__(self, cfg: TransformerConfig):
        ...
        self.pos_proj = nn.Linear(cfg.pos_feat_dim, cfg.heads * cfg.pair_dim)  
    ...
    def _avg_pool_runtime(self, x: torch.Tensor, target_P: int) -> torch.Tensor:
        # x: [B, S, C] -> [B, P, C], with P ≈ target_P (but adapts)
        B, S, C = x.shape
        stride = max(1, S // max(1, target_P))  # ensures >=1
        x_ds = F.avg_pool1d(x.transpose(1, 2), kernel_size=stride, stride=stride).transpose(1, 2)
        return x_ds


    def forward(self, x: torch.Tensor):
        B, S, C = x.shape
        target_P = self.cfg.target_pair_len
        
        x_ds = self._avg_pool_runtime(x, target_P)                                                  # [B, P, C]                               
        P = x_ds.shape[1]

        #  Project to Query and Key
        q = self.q_proj(x_ds).reshape(B, P, self.cfg.heads, self.cfg.pair_dim)                        # [B, P, H, C] = [B, 8, 8, 64]
        k = self.k_proj(x_ds).reshape(B, P, self.cfg.heads, self.cfg.pair_dim)                        # [B, P, H, C] = [B, 8, 8, 64]

        # print(f"P: {P}")  # Should be 32 in this case
        pos_features = central_mask_features(P, self.cfg.pos_feat_dim).to(x.device)                # [2P-1, 64] = [15, 64]
        pos_enc = self.pos_proj(pos_features).reshape(
            2 * P - 1, self.cfg.heads, self.cfg.pair_dim
        )  # [15, 8, 64]

        # q + q_bias = [B, Q, H, C] = [B, 8, 8, 64]
        # pos_enc = [2P-1, H, C] = [15, 8, 64]
        # einsum: [B, Q=8, H=8, D=64], [P=15, H=8, D=64] → [B, Q, P, H] = [B, 8, 15, 8]
        rel_q_a = relative_shift(torch.einsum('bqhc,phc->bqph', q + self.q_bias, pos_enc))  # [B,P,P,H]  # Output = [B, 8, 8, 8]
        # print("rel_q_a:", rel_q_a.shape)
        rel_k_a = relative_shift(torch.einsum('bkhc,phc->bkph', k + self.k_bias, pos_enc))  # [B,P,P,H]  # Output = [B, 8, 8, 8]
        ...
        # [B, P, P, pair_dim]
        return pair_activations, P, S                                                              # [B, 8, 8, 64]
...
def central_mask_features(sequence_length: int, feature_size: int):
    ...
    assert feature_size % 2 == 0, "pos_feat_dim must be even"
    rel_pos = torch.arange(2 * sequence_length - 1) - (sequence_length - 1)                  # [2*sequence_length - 1]
    end_val = max(1, sequence_length - feature_size // 2 + 1) 
    widths = torch.arange(feature_size // 2, device=rel_pos.device) + torch.logspace(
        start=0,
        end=math.log10(end_val),
...
class AttentionBiasBlock(nn.Module):
    def __init__(self, cfg: TransformerConfig):
        super().__init__()
        self.linear = nn.Linear(cfg.pair_dim, cfg.heads, bias=False)

    def forward(self, pair_x: torch.Tensor, P: int, S: int):
        """
        Args:
        ...
        """
        # pair_x: [B,P,P,pair_dim] -> [B,H,S,S]
        bias_pph = self.linear(F.gelu(pair_x))               # [B,P,P,H]
        bias_bhpp = bias_pph.permute(0, 3, 1, 2)             # [B,H,P,P]
        if P != S:
            return F.interpolate(bias_bhpp, size=(S, S), mode="nearest")
        return bias_bhpp
